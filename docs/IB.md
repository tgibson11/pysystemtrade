...
* [Preliminaries](#preliminaries)
   * [Getting started with interactive brokers](#getting-started-with-interactive-brokers)
      ...
      * [ib_async library](#ib_async-library)
      * [IBC](#ibc)
...
I use [ib_async](https://github.com/ib-api-reloaded/ib_async) as my interface to the IB API. This library is a required dependency, and is an actively maintained fork of the archived ib-insync library.

It is worth running the examples in the [ib_async recipes](https://ib-api-reloaded.github.io/ib_async/recipes.html) to make sure your IB connection is working, that you have the right gateway settings, and so on. Pysystemtrade obviously won't work if ib_async can't work!!
...
- Client objects: These make calls to the ib_async library in specific domains (getting data, placing orders and so on). They are also instanced with a *connection object*.
- Connection objects. These contain a specific connection to an IB gateway via an ib_async `IB` instance.
...
Client objects make calls and requests to the broker via ib_async. They are usually initialised by a broker data source object, which passes them a connection (and optionally a log).
...
You can use this directly if you are familiar with ib_async eg `conn.ib.positions()`, but normally this would be used by the IB client objects.
