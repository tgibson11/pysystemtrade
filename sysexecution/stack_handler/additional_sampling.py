...
class stackHandlerAdditionalSampling(stackHandlerCore):
    ...
    def _get_all_instruments_priced_contracts_from_db(self):
        ...
        instrument_list_excluding_ignored = remove_ignored_instruments(
...
def remove_ignored_instruments(
    self: stackHandlerAdditionalSampling, list_of_instruments: List[str]
):
    ignore_list = self.data.config.get_element("exclude_instrument_lists").get(
        "ignore_in_order_stack", []
    )
    ...
    new_list = [
        ...
        if not instrument in ignore_list
    ]
